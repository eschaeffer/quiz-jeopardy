...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!payload?.model || !payload?.topic || !payload?.subjectFamily) {
      throw createHandledError(400, 'model, topic, and subjectFamily are required', 'BAD_REQUEST', 'request_error');
    }

    return jsonResponse(200, await requestFinalGeneration({
    ...
    return errorResponse(
      ...
      error.message,
      error.code || 'FINAL_GENERATION_ERROR',
